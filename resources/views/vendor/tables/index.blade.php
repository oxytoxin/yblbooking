...
<div x-data="{
    ...
    <x-tables::container>
        <div x-show="hasHeader = ({{ ($renderHeader = ($header || $heading || $headerActions || $isSearchVisible || $isFiltersDropdownVisible || $isColumnToggleFormVisible)) ? 'true' : 'false' }} || selectedRecords.length)"
            ...
            <div class="px-2 pt-2 space-y-2">
                ...
                <x-tables::hr
                    x-show="{{ ($isSearchVisible || $isFiltersDropdownVisible || $isColumnToggleFormVisible) ? 'true' : 'false' }} || selectedRecords.length" />
            </div>
            @endif

            <div x-show="{{ ($renderHeaderDiv = ($isSearchVisible || $isFiltersDropdownVisible || $isColumnToggleFormVisible)) ? 'true' : 'false' }} || selectedRecords.length"
                ...
                <div class="w-full flex items-center justify-end gap-2 md:max-w-md">
                    ...
                    <x-tables::filters :form="$getFiltersForm()" :width="$getFiltersFormWidth()" class="shrink-0" />
                    @endif

                    @if ($isColumnToggleFormVisible)
                    <x-tables::toggleable :form="$getTableColumnToggleForm()" :width="$getTableColumnToggleFormWidth()"
                        class="shrink-0" />
                    @endif
                </div>
                @endif
        ...
        <div @class([ 'overflow-y-auto relative' , 'dark:border-gray-700'=> config('tables.dark_mode'),
            ...
            <x-tables::table>
                ...
                @if ($isSelectionEnabled())
                <x-tables::selection-indicator :all-records-count="$getAllRecordsCount()" :colspan="$columnsCount"
                ...
                <x-tables::row :record-url="$getRecordUrl($record)" wire:key="{{ $record->getKey() }}" x-bind:class="{
                    ...
                    <x-tables::cell :action="$column->getAction()" :name="$column->getName()"
                        :alignment="$column->getAlignment()" :record="$record" :tooltip="$column->getTooltip()"
                        :record-action="$getRecordAction()" :record-url="$getRecordUrl($record)"
                        :should-open-url-in-new-tab="$column->shouldOpenUrlInNewTab()" :url="$column->getUrl()"
        ...
        <div @class([ 'p-2 border-t' , 'dark:border-gray-700'=> config('tables.dark_mode'),
            ])>
