...
<div x-data="{
    ...
    <div x-show="isOpen" x-on:click.away="isOpen = false" x-transition:enter="transition"
        ...
        <ul @class([ 'py-1 space-y-1 overflow-hidden bg-white shadow rounded-xl'
            ...
            <x-tungsten.dropdown.item :color="$accountItem?->getColor() ?? 'secondary'"
                :icon="$accountItem?->getIcon() ?? 'heroicon-s-user-circle'" :href="$accountItem?->getUrl()" tag="a">
                {{ $accountItem?->getLabel() ?? auth()->user()->name }}
            </x-tungsten.dropdown.item>

            <div>
                @if (config('filament.dark_mode'))
                <x-tungsten.dropdown.item icon="heroicon-s-moon" x-show="theme === 'dark'"
                    x-on:click="mode = 'manual'; theme = 'light'">
                    {{ __('filament::layout.buttons.light_mode.label') }}
                </x-tungsten.dropdown.item>

                <x-tungsten.dropdown.item icon="heroicon-s-sun" x-show="theme === 'light'"
            ...
            <x-tungsten.dropdown.item :color="$item->getColor() ?? 'secondary'" :icon="$item->getIcon()"
                :href="$item->getUrl()" tag="a">
                {{ $item->getLabel() }}
            </x-tungsten.dropdown.item>
            @endif
            @endforeach

            <x-tungsten.dropdown.item :color="$logoutItem?->getColor() ?? 'secondary'"
                :icon="$logoutItem?->getIcon() ?? 'heroicon-s-logout'" :action="route('logout')" method="post"
                tag="form">
