...
<li class="filament-sidebar-item">
    <a href="{{ $url }}" {!! $shouldOpenUrlInNewTab ? 'target="_blank"' : '' !!}
        ...
        @if(config('filament.layout.sidebar.is_collapsible_on_desktop')) x-data="{ tooltip: {} }" x-init="
                Alpine.effect(() => {
        ...
        @class([ 'flex items-center justify-center gap-3 px-3 py-2 rounded-lg font-medium transition'
        ...
        <div class="flex flex-1" @if (config('filament.layout.sidebar.is_collapsible_on_desktop')) x-data="{}"
