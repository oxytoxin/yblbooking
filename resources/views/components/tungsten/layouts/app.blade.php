<x-tungsten.layouts.base :title="config('app.name')">
    <div class="flex min-h-screen w-full filament-app-layout">
        ...
        <x-tungsten.layouts.app.sidebar :items="[]" />
                ...
                <header @class([ 'h-[4rem] shrink-0 w-full border-b flex items-center filament-main-topbar'
                    ...
                    <div @class([ 'flex items-center w-full px-2 mx-auto sm:px-4 md:px-6 lg:px-8' , match
                        ...
                        <button x-data="{}"
                            ...
                            @class([ 'shrink-0 flex items-center justify-center w-10 h-10 text-primary-500 rounded-full filament-sidebar-open-button hover:bg-gray-500/5 focus:bg-primary-500/10 focus:outline-none'
                            , 'lg:hidden'=> ! config('filament.layout.sidebar.is_collapsible_on_desktop'),
                            'lg:mr-4 rtl:lg:mr-0 rtl:lg:ml-4' =>
                        ...
                        <div class="flex-1 flex gap-4 items-center justify-between">
                            @isset($breadcrumbs)
                            <x-tungsten.layouts.app.topbar.breadcrumbs :breadcrumbs="$breadcrumbs" />
                            @endisset
                            @livewire('filament.core.global-search')

                            <x-tungsten.layouts.app.topbar.user-menu />
                        </div>
                    </div>
                </header>
